...
pkg_install musl-openssl-1.0.1j-1 || exit 2
pkg_install musl-popt-1.16-1 || exit 2
pkg_install musl-libnl-1.1-1 || exit 2
...
NETSNMP_CONFIG=/opt/musl/bin/net-snmp-config LIBS="-lm" \
 $PKGDIR/B-configure-3 --prefix=/opt/keepalived --disable-lvs --enable-snmp --enable-sha1 || exit 1
