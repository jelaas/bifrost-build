...
VER=3.0
SRCVER=gnu-efi_${VER}w
PKG=musl-$SRCVER-1 # with build version

# PKGDIR is set by 'pkg_build'. Usually "/var/lib/build/all/$PKG".
PKGDIR=${PKGDIR:-/var/lib/build/all/$PKG}
SRC=/var/spool/src/$SRCVER.orig.tar.gz 
BUILDDIR=/var/tmp/src/gnu-efi-$VER
...
sed -i 's,/usr/local,/opt/musl,' Make.defaults
sed -i 's/gcc/musl-gcc/' Make.defaults
...
#########
# Compile
make PREFIX=/opt/musl INSTALLROOT=$DST || exit 1

#########
# Install into dir under /var/tmp/install
rm -rf "$DST"
make install INSTALLROOT=$DST DESTDIR=$DST # --with-install-prefix may be an alternative
