...
cd $BUILDDIR || exit 1
libtool_fix-1
patch -p0 < $PKGDIR/plugins.pat || exit 1
sed -i 's/MSG_TRYHARD/MSG_DONTROUTE/' bcrelay.c

#########
# Configure
OPTPREFIX=opt/pptpd
B-configure-3 --prefix=/$OPTPREFIX --localstatedir=/var --with-libwrap --enable-bcrelay || exit 1
...
mkdir -p $OPTDIR/rc.d
echo yes > $OPTDIR/etc/config.flags/pptpd
echo $PKG > $OPTDIR/pkgversion
cp -p $PKGDIR/rc $OPTDIR/rc.d/rc.pptpd
chmod +x $OPTDIR/rc.d/rc.pptpd
cp README.* $OPTDIR
cp -r samples tools $OPTDIR
