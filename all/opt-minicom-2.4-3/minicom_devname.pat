...
+	     P_LOCK, mbasename(devname_get(dial_tty), buf, sizeof(buf)));
 #endif /* SVR4_LOCKS */
...
+	    portfd = open(devname_get(dial_tty), O_RDWR|O_NDELAY|O_NOCTTY);
       if (portfd >= 0) {
...
+	      portfd = open(devname_get(dial_tty), O_RDWR|O_NOCTTY);
 #endif
     }
     if (portfd >= 0) {
