...
sedit 's/lncursesw/lncurses -static/g' Makefile
sedit 's/@(cd/#/g' Makefile
...
#########
# Compile
CFLAGS="-Os -march=i586 -Wall" make || exit 1
...
mkdir -p $DST/usr/bin
strip powertop
cp -p powertop $DST/usr/bin

#########
# Check result
cd $DST
# [ -f usr/bin/myprog ] || exit 1
(ldd usr/bin/powertop|grep -qs "not a dynamic executable") || exit 1
