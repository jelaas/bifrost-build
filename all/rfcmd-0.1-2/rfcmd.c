...
int main( int argc, char **argv )
{
	...
	if( (argc == 6) && (strcmp(*(argv+2), "NEXA") == 0))
	{
		...
		if (createNexaString(*(argv+3), *(argv+4), *(argv+5), txStr) == 0)
		{
			printUsage();
			exit(1);
		}
	...
	if( 0 > ( fd = open(devname_get(*(argv+1)), O_RDWR ) ) )
	{
...
void printUsage(void)
{
	...
	printf("\t PROTOCOLS: NEXA\n" );
	printf("\t PROTOCOL ARGUMENTS - NEXA: \n");
