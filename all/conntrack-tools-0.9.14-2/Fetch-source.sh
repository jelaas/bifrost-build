...
[ -s "${DST}" ] || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://www.netfilter.org/projects/conntrack-tools/files/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://pkgs.fedoraproject.org/repo/pkgs/conntrack-tools/conntrack-tools-0.9.14.tar.bz2/7e9344fe85bb68bcf65c35034add6655/"${SRC}" \
                ...
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://piotrkosoft.net/pub/mirrors/ftp.netfilter.org/conntrack-tools/"${SRC}" \
