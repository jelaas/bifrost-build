...
VER=1.36
SRCVER=fossil-src-$VER
PKG=opt-fossil-$VER-1 # with build version

# PKGDIR is set by 'pkg_build'. Usually "/var/lib/build/all/$PKG".
PKGDIR=${PKGDIR:-/var/lib/build/all/$PKG}
SRC=/var/spool/src/$SRCVER.tar.gz
[ -f /var/spool/src/$SRCVER.tar.bz2 ] && SRC=/var/spool/src/$SRCVER.tar.bz2
BUILDDIR=/var/tmp/src/fossil-$VER
...
pkg_install musl-openssl-1.1.0b-1 || exit 2
pkg_install daemon-1.0-1 || exit 2
export CC=musl-gcc
...
#########
# Configure
OPTPREFIX=opt/fossil
B-configure-3 --prefix=/$OPTPREFIX --localstatedir=/var --static || exit 1
...
mkdir -p $OPTDIR/rc.d
mkdir -p $OPTDIR/repos
echo yes > $OPTDIR/etc/config.flags/fossil
echo $PKG > $OPTDIR/pkgversion
cp -p $PKGDIR/rc $OPTDIR/rc.d/rc.fossil
cp $PKGDIR/options.nossl $OPTDIR/etc/config.data
chmod +x $OPTDIR/rc.d/rc.fossil
cp /bin/daemon $OPTDIR/bin || exit 1
