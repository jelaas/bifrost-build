...
sed -i 's/-Wl,--export-dynamic//' configure || exit 1

#########
# Configure

OPTPREFIX=opt/pmacct
B-configure-2 --disable-so --prefix=/$OPTPREFIX --localstatedir=/var || exit 1
...
(ldd opt/pmacct/bin/pmacct|grep -qs "not a dynamic executable") || exit 1
