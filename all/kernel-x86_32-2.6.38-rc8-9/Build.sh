...
# patch -p1 < $PKGDIR/slub_nomerge.pat || exit 1

patch -p0 < $PKGDIR/dev_c_remove_module_spam.pat || exit 1
patch -p0 < $PKGDIR/niu_rx_channel.pat || exit 1
patch -p0 < $PKGDIR/niu_rx_bytes_2.pat || exit 1

# Backported patches
patch -p1 < $PKGDIR/backport_net_ipv4_fib_trie.pat || exit 1
patch -p1 < $PKGDIR/backport_net_ipv4_raw.pat || exit 1
patch -p1 < $PKGDIR/tcp_output.pat || exit 1
patch -p1 < $PKGDIR/inetpeer_c.pat || exit 1
patch -p1 < $PKGDIR/niu_64bit_stats.pat || exit 1
patch -p1 < $PKGDIR/backport_niu_hw_features.pat || exit 1
