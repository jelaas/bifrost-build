...
pkg_install perl-5.10.1-3 || exit 1
...
sed -i 's/A-Z0-9/ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789/' build/make/rtcd.sh
sed -i 's/soft_enable ssse3/soft_disable ssse3/' build/make/configure.sh
sed -i 's/ssse3//' ./vp8/common/rtcd_defs.sh

#########
# Configure
./configure --enable-static --disable-shared --prefix=/opt/musl --disable-docs --disable-examples --disable-unit-tests --enable-runtime-cpu-detect || exit 1
[ -f config.log ] && cp -p config.log /var/log/config/$PKG-config.log

#########
# Post configure patch
# patch -p0 < $PKGDIR/Makefile.pat
sed -i 's/HAVE_SSSE3 1/HAVE_SSSE3 0/' vpx_config.h
sed -i 's/HAVE_SSSE3=yes/HAVE_SSSE3=no/' libs-x86-linux-gcc.mk
