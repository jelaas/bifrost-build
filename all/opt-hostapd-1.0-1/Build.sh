...
CFLAGS="-Wall -Os -march=i586 -I/usr/include/libnl3" V=1 make || exit 1
...
(ldd opt/hostapd/bin/hostapd|grep -qs "not a dynamic executable") || exit 1
(ldd opt/hostapd/bin/hostapd_cli|grep -qs "not a dynamic executable") || exit 1
