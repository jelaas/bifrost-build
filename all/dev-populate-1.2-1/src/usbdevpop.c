/*
 * File: usbdevpop.c
 * Implements: automatic population of usb devices as character device  nodes
 *
...
root@laas:~# cat /sys/class/usb_device/usbdev1.3/dev
189:2
root@laas:~# ls -l /dev/bus/usb/001/003 
crw-rw-rw- 1 root root 189, 2 Sep 21 21:37 /dev/bus/usb/001/003

usbdev1.3 => /dev/bus/usb/001/003

Ovanstående använder DEPRECATED USB_DEVICE_CLASS.



*/

FIXME: mount usbfs on /proc/bus/usb. use /proc/bus/usb to generate /dev info

 seems hard. hmm. looks like maybe the usbfs entries can be used inplace of
 the /dev/bus/usb entries.
 So maybe just mount usbfs on /dev/bus/usb ?

and then this program wont be needed! Yoho!
(we could still be useful for setting ownerships and access modes on the files
 in usbfs. but thats it)
...
int sysusb_scan()
{
	...
	char buf[16], fn[64];
	ssize_t n;
	int major, minor, fd, hi, low;
	
	devs = jl_new();
	
	sprintf(fn, "%s/class/usb_device", conf.sysdir);
	find_devs(devs, fn);
	...
	jl_foreach(devs, s) {
		...
		if(fd != -1) {
			if((n=read(fd, buf, sizeof(buf)-1))>0) {
				...
				sprintf(fn, "%s/bus/usb/%03d",
					conf.devdir,
					hi);
				mkdir(fn, 0777);
				sprintf(fn, "%s/bus/usb/%03d/%03d",
					conf.devdir,
					hi,
					low);
				if(conf.debug)
