...
[ -s "${DST}" ] || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://www.skbuff.net/iputils/"${SRC}" \
                ...
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://dev.gateworks.com/sources/?C=M;O=A"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" https://www.jabawok.net/gentoo/distfiles/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" https://downloads.yoctoproject.org/releases/yocto/yocto-1.5/sources/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://mirror.yandex.ru/slackware/slackware64-13.1/source/n/iputils/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://dl.openwrtdl.com/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://ftp.tw.netbsd.org/Linux/slackware/slackware-13.1/source/n/iputils/"${SRC}" \
