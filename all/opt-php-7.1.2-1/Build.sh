...
SRCVER=php-7.1.2
PKG=opt-php-7.1.2-1 # with build version
...
pkg_install curl-devel-7.51.0-1 || exit 2
...
#########
# Configure
LIBS="-lpthread" B-configure-1 --prefix=/opt/php-7.1 \
...
echo $PKG > $DST/opt/php-7.1/pkgversion
cp php.ini-production $DST/opt/php-7.1/bin
...
(ldd opt/php-7.1/bin/php|grep -qs "not a dynamic executable") || exit 1

#########
# Clean up
cd $DST || exit 1
rm -rf opt/php-7.1/include opt/php-7.1/lib opt/php-7.1/var
strip opt/php-7.1/bin/*
