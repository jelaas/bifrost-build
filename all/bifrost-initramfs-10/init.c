...
int main(int argc, char **argv, char **envp)
{
	...
	int retries = 0;
	int rootmounted = 0;
	unsigned long mountflags = MS_SYNCHRONOUS;
	char *rootdev = NULL;
	...
	if(cmdline.rootfstype) {
		...
		if(mount(rootdev, "/rootfs", cmdline.rootfstype, MS_NOATIME|MS_RDONLY|mountflags, "")) {
	...
	if(!rootmounted) {
		...
		if(mount(rootdev, "/rootfs", "ext2", MS_NOATIME|MS_RDONLY|mountflags, "")) {
