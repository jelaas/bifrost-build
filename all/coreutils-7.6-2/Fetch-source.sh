...
[ -s "${DST}" ] || ../../wget-finder --checksum "${MD5}" -O "${DST}" ftp://ftp.sunet.se/pub/gnu/coreutils/"${SRC}" \
                ...
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://gnu.raffsoftware.com/coreutils/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://mirror.clarkson.edu/gnu/coreutils/"${SRC}" \
                || ../SRC../wget-finder -O "${DST}" "${SRC}:${MD5}"
