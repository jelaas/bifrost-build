...
[ -s "${DST}" ] || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://downloads.sourceforge.net/project/e2fsprogs/e2fsprogs/v1.42.5/"${SRC}" \
                ...
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://sources.openelec.tv/mirror/e2fsprogs/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://repository.timesys.com/buildsources/e/e2fsprogs/e2fsprogs-1.42.5/"${SRC}" \
