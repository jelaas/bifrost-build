...
cd $BUILDDIR || exit 1
libtool_fix-1
patch -p0 < $PKGDIR/configure.ac.pat || exit 1


cd $BUILDDIR || exit 1
./bootstrap.sh

sed -i 's/WANTS_GTK=yes/WANTS_GTK=no/' configure
sed -i 's/WANTS_GLIB=yes/WANTS_GLIB=no/' configure
