...
[ -s "${DST}" ] || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://ftp.sunet.se/pub/Linux/distributions/bifrost/download/src/"${SRC}" \
                ...
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://debian.hro.nl/kernel.org/linux/utils/net/iproute2/"${SRC}" \
                || ../../wget-finder --checksum "${MD5}" -O "${DST}" http://ftp.kfki.hu/linux/ftp.kernel.org/pub/linux/utils/net/iproute2/"${SRC}" \
