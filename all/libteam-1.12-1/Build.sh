...
pkg_install musl-libdaemon-0.14-1 || exit 2
pkg_install musl-jansson-2.7-1 || exit 2
...
# Configure
./autogen.sh || exit 1
LIBDAEMON_CFLAGS=-Os LIBDAEMON_LIBS=-ldaemon B-configure-3 --disable-dbus --disable-zmq --prefix=/ || exit 1
...
(ldd bin/teamd|grep -qs "not a dynamic executable") || exit 1
