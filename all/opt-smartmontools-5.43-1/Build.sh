...
#########
# Configure
OPTPREFIX=opt/smartmontools
B-configure-2 --prefix=/$OPTPREFIX --localstatedir=/var || exit 1
...
mkdir -p $OPTDIR/rc.d
echo yes > $OPTDIR/etc/config.flags/smartd
echo $PKG > $OPTDIR/pkgversion
cp -p $PKGDIR/rc $OPTDIR/rc.d/rc.smartd
chmod +x $OPTDIR/rc.d/rc.smartd
[ -f $PKGDIR/README ] && cp -p $PKGDIR/README $OPTDIR
mv $OPTDIR/etc/smartd.conf $OPTDIR/etc/config.preconf
