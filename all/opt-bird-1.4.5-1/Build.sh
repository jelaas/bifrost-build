...
# Configure
OPTPREFIX=opt/bird
$PKGDIR/B-configure-3 --prefix=/$OPTPREFIX --localstatedir=/var --enable-ipv6 || exit 1
...
mkdir -p $OPTDIR/rc.d
echo yes > $OPTDIR/etc/config.flags/bird
echo $PKG > $OPTDIR/pkgversion
cp -p $PKGDIR/rc $OPTDIR/rc.d/rc.bird
chmod +x $OPTDIR/rc.d/rc.bird
[ -f $PKGDIR/README ] && cp -p $PKGDIR/README $OPTDIR
mv $OPTDIR/etc/bird6.conf $OPTDIR/etc/config.preconf
