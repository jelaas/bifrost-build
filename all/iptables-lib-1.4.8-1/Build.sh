...
SRCVER=iptables-1.4.8
PKG=iptables-lib-1.4.8-1 # with build version
...
(ldd usr/sbin/iptables-multi|grep -qs "not a dynamic executable") || exit 1

#########
# Clean up
cd $DST
rm -rf usr/sbin usr/bin usr/share usr/libexec
