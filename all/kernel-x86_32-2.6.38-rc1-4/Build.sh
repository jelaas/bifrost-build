...
SRCVER=kernel-$V
BUILDVERSION=4
PKG=kernel-x86_32-$V-$BUILDVERSION # with build version
...
sed -i "s/bifrost-x86_32/${BUILDVERSION}-bifrost-x86_32/" .config
...
[ -d lib/modules/$V-$BUILDVERSION-bifrost-x86_32/kernel ] || exit 1

find lib/modules/$V-$BUILDVERSION-bifrost-x86_32/kernel|filter_modules
