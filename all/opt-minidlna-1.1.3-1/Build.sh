...
SRC=/var/spool/src/minidlna-${VER}.tar.gz
...
LIBS="-lvorbis -logg -lm -lsqlite3 -lpthread -lexif -ljpeg -lFLAC -lm -lid3tag -lz -lavformat -lavcodec -lavutil -lm" B-configure-3 --prefix=/$OPTPREFIX --localstatedir=/var || exit 1
...
#########
# Compile
make V=1 INSTALLPREFIX=$DST/$OPTPREFIX DESTDIR=$DST || exit 1
...
(ldd opt/minidlna/sbin/minidlnad|grep -qs "not a dynamic executable") || exit 1
